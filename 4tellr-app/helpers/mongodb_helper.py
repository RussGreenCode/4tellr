...
class MongoDBHelper:
    def __init__(self, config):
        ...
        self.event_collection = self.db[config['EVENT_COLLECTION']]
        self.stats_collection = self.db[config['STATS_COLLECTION']]
        self.monitoring_groups_collection = self.db[config['MONITORING_GROUPS_COLLECTION']]
        self.user_collection = self.db[config['USER_COLLECTION']]
    ...
    def insert_event_outcome(self, event_data):
        ...
        query = {
            ...
            'eventId': {'$regex': f'^EXP#{event_name}#{event_status}'}
        }
        items = list(self.event_collection.find(query))
        ...
        self.event_collection.insert_one(outcome_data)
    ...
    def query_event_details(self, event_id):
        ...
        return self.event_collection.find_one(query)
    ...
    def get_expected_time(self, event_name, event_status):
        query = {'event_name_and_status': event_name + '#' + event_status}
        return self.stats_collection.find_one(query)

    def get_latest_metrics(self):
        # Perform a find to get all items
        items = list(self.stats_collection.find())

        # Dictionary to keep track of the latest item for each event_name_and_status
        latest_metrics = {}
        for item in items:
    ...
    def generate_expectations(self, business_date):
        ...
        for metric in latest_metrics:
            ...
            self.event_collection.insert_one(expectation_data)
    ...
    def delete_expectations_for_business_date(self, business_date):
        try:
            ...
            result = self.event_collection.delete_many(query)
            self.logger.info(f'Deleted {result.deleted_count} items for business date {business_date}')
        except Exception as e:
            self.logger.error(f'Error deleting items for business date {business_date} - {str(e)}')
            raise
    ...
    def delete_events_for_business_dates(self, business_dates):
        for business_date in business_dates:
            try:
                query = {
                    'eventId': {'$regex': '^(EVT#|OUT#)'},
                    'businessDate': business_date
                }
                result = self.event_collection.delete_many(query)
                self.logger.info(f'Deleted {result.deleted_count} events for business date {business_date}')
            except Exception as e:
                self.logger.error(f'Error deleting items for business date {business_date} - {str(e)}')
                raise
    ...
    def get_monthly_events(self, event_name, event_status):
        ...
        query = {
            'businessDate': {'$gte': start_date.strftime('%Y-%m-%d'), '$lte': end_date.strftime('%Y-%m-%d')},
            'eventId': {'$regex': f'^EVT#{event_name}#{event_status}'}
        }
    ...
    def save_group(self, group_name, events, description):
        if group_name and events is not None:
            try:
                response = self.monitoring_groups_collection.insert_one({
    ...
    def update_group(self, group_name, new_events):
        if group_name and new_events is not None:
            try:
                response = self.monitoring_groups_collection.update_one(
                    {'group_name': group_name},
    ...
    def delete_group(self, group_name):
        if group_name:
            try:
                response = self.monitoring_groups_collection.delete_one({'group_name': group_name})
    ...
    def get_all_groups(self):
        try:
            response = self.monitoring_groups_collection.find(
                {},
                {'_id': 0, 'group_name': 1, 'description': 1}
            )
    ...
    def get_group_details(self, group_name):
        if group_name is not None:
            try:
                response = self.monitoring_groups_collection.find_one(
                    {'group_name': group_name}
                )
                if response:
    ...
    def add_user(self, user):
        ...
        try:
            ...
            response = self.user_collection.find_one({'email': email})

            if response:
                return {'email': email, 'success': False, 'message': 'User with this email already exists'}

            # Add the new user (including the salt)
    ...
    def get_all_users(self):
        try:
            response = self.user_collection.find(
                {},
                {'_id': 0, 'email': 1}
            )
            users = list(response)
    ...
    #        new_hashed_password = bcrypt.hashpw(new_password.encode('utf-8'), new_salt.encode('utf-8')).decode('utf-8')
    #        try:
    #            self.user_collection.update_one(
    #                {'email': email},
    #                {'$set': {'password': new_hashed_password, 'salt': new_salt}}  # Update salt as well
    #            )
    ...
    def change_password(self, email, old_password, new_password):
        ...
        if user:
            ...
            salt = user.get('salt')
            if salt:
                # Hash the provided old password using the stored salt
                hashed_old_password = bcrypt.hashpw(old_password.encode('utf-8'), salt.encode('utf-8')).decode('utf-8')
                if bcrypt.checkpw(old_password.encode('utf-8'), hashed_old_password.encode('utf-8')):  # Compare against stored hashed password
                    # Generate a new salt for the new password
                    new_salt = bcrypt.gensalt().decode('utf-8')
                    new_hashed_password = bcrypt.hashpw(new_password.encode('utf-8'), new_salt.encode('utf-8')).decode('utf-8')
                    try:
